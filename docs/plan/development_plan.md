...
4. UI 视觉已重新进入设计阶段（v2.0），方向从"竞技桌垫"翻为"独立数字面板 + MD 冷蓝黑基底"，详见 `docs/plan/ui_art_style_guide.md` v2.0 与 `docs/adr/0001-digital-panel-ui-direction.md`。
5. 对 `tools/compile_cards_effects.py` 的后续识别层重构，默认以“同类文本参数化复用同一模板族”为约束，不接受仅把逐句匹配搬入注册表的半重构状态。
6. `compile_cards_effects` 优化路线已于 2026-04-17 完成 P4（质量门禁与长期治理）：冲突增量阻断、新冲突键阻断、规则顺序漂移阻断与高优先级 family 冲突回升阻断均已接入 `check_compiler_metrics_gate.py`，并由对应单元测试守护。
...
- 将战场 UI 从"竞技桌垫印刷分区"全面翻新为"独立数字面板"体系
- 以 Master Duel 冷蓝黑赛博竞技场为视觉参考
- 建设 `battle_theme.theme` + 双字体（Noto Sans SC / Barlow Condensed）
- 拆分 `BattleScene`（脚本 + 场景双拆）、重建布局、补动效/tooltip/日志

本阶段已冻结的设计决策（37 条）见 `docs/adr/0001-digital-panel-ui-direction.md`，
完整视觉规范见 `docs/plan/ui_art_style_guide.md` v2.0，
领域术语词汇表见 `CONTEXT.md`。

推荐执行顺序：

1. **P0 — Theme 与字体**（改动最小、无回归风险）— 建 `battle_theme.theme`，加载字体，迁移现有颜色到 Theme
2. **P1 — 骨架重构**（最大改动）— 拆 `BattleScene`（脚本+场景），建通用面板 `.tscn`，`ZoneLayoutConfig` 退役
3. **P2 — 布局落地** — 四行镜像面板排列，附属堆叠，顶部 HUD，手牌/预览面板重新定位
4. **P3 — 动效与交互增强** — 入场/离场动画，tooltip 系统，日志抽屉，inline 错误提示
5. **P4 — 素材补全** — 最小素材集（图标、能量圆点），面板纹理（可选）

约束：

- `core/` 规则层和 UI 快照接口不变（只改 UI 层消费方式）
- 头两个 Phase（P0 + P1）完成后不可回退到旧布局
- 每 Phase 结束后同步更新当日日志
- 若触及 `scenes/battle_scene.tscn`，必须补布局验收确认手牌不遮挡战场

完成判据：

- `battle_theme.theme` 存在且所有 UI 颜色/字体/圆角/间距可追溯
- `BattleScene` ≤ 300 行，职责限于信号路由和快照分发
- 四行面板在 1440p 和 1080p 下均能正确渲染
- 所有现有冒烟测试持续通过（UI 层改动不破坏规则层）
- 手牌区域不遮挡战场面板
