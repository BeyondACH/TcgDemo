...
- `Week 3` 当前执行进度：基线已继续收敛到 `Unsupported abilities: 0`，`Phase 4` 目标（`8 -> 0`）已达成；本轮补齐剩余长尾与专项 override，覆盖了“能量阈值目标授予不可阻挡”“动态阈值按前线低费数量扩展”“手札复合条件登场 + 条件自增益”“来源 BP 比较退场”等尾项，已进入 `Phase 5` 并启用数据回归门禁（`tests/test_phase5_regression_guards.py` + `check_unsupported_budget` 零基线）。
