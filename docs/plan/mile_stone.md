...
- 当前执行状态：P2 首轮并行实施已落地，已按“规则专项线 + raw 样例线”拆开收口。
- 规则专项线：
  - `docs/milestone_smoke_test.gd` 继续承担规则主冒烟，不混入正式 raw 卡编号样例。
  - 已新增“同时触发顺序”专项回归，覆盖：
    - 同一方多个同时触发时进入 `TRIGGER_ORDER` 待决策，并允许按玩家选择顺序逐项处理。
    - 双方同时触发时，按 `docs/rules/rule.md` 10.3 先处理回合方，再处理非回合方。
- raw 样例线：
  - `docs/cards_raw_minimal_duel_smoke_test.gd` 继续承担正式 raw 卡样例稳定性验证，不重复证明规则主语义。
  - 本轮已补 8 条新增样例，覆盖：
    - 临时产能修饰：临时产能可支撑后续出牌、到期后不再放行额外出牌。
    - 延迟自退场：离场后产能贡献会同步清理、多个延迟自退场并存时不破坏阶段推进。
    - 临时特殊登场 / `RAID` 许可：许可绑定具体实例、不外溢到其他同卡副本，且仍受 `RAID` 底座合法性约束。
    - 看牌堆顶后的条件追加结算：已选卡不会重新进入后续候选，跳过奖励分支时仍保持回底顺序契约。
- 当前职责边界：
  - `docs/milestone_smoke_test.gd`：规则语义、触发顺序、主冒烟基线。
  - `docs/cards_raw_minimal_duel_smoke_test.gd`：正式 raw 卡样例、模板级稳定性回归。
- 当前验证结果：
...
1. 保持 `docs/milestone_smoke_test.gd` 作为规则主冒烟入口，后续高风险规则区默认先接入这里。
2. 继续在 `docs/cards_raw_minimal_duel_smoke_test.gd` 扩正式 raw 样例，优先补更复杂的预览链、多段条件追加结算与后续模板回归。
3. 仅当预览链样例继续明显膨胀时，再考虑拆出单独的 raw 预览专项脚本；当前阶段暂不拆分。
