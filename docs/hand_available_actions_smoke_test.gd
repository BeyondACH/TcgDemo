...
func _init() -> void:
	...
	_run_test("RAID 卡 life_trigger_only 仍可在主阶段从手牌显示", _test_raid_life_trigger_only_available_in_main)
	_run_test("正式 life_trigger_only RAID 卡在主阶段也可从手牌 RAID", _test_official_life_trigger_only_raid_available_in_main)
...
## 测试：RAID 卡 life_trigger_only=true 时仍可在主阶段从手牌显示 RAID
func _test_raid_life_trigger_only_available_in_main() -> Dictionary:
	var manager := _new_manager()
	_advance_to_main(manager, UATypes.PLAYER_ONE)
	var p1 := _player(manager, UATypes.PLAYER_ONE)
	p1.ap_area = [{"index": 0, "active": true}]
	...
	var actions := _get_hand_card_actions(manager, UATypes.PLAYER_ONE, raid_uid)
	if not actions.has("RAID"):
		return _fail("life_trigger_only=true 的 RAID 卡在主阶段满足条件时仍应显示 RAID 动作，实际动作: %s" % str(actions))

	return _ok()

## 测试：正式 life_trigger_only RAID 卡在主阶段也可从手牌 RAID
func _test_official_life_trigger_only_raid_available_in_main() -> Dictionary:
	var manager := _new_manager()
	_advance_to_main(manager, UATypes.PLAYER_ONE)
	var p1 := _player(manager, UATypes.PLAYER_ONE)
	p1.ap_area = [{"index": 0, "active": true}]
	for i in range(5):
		var energy_uid := _spawn_temp_card(manager, UATypes.PLAYER_ONE, {
			"id": "TMP_OFFICIAL_LIFE_RAID_ENERGY_%d" % i,
			"name": "正式生命RAID测试能量%d" % i,
			"card_type": "CHARACTER",
			"title_code": "TMP",
			"number": "TMP-OLR-%d" % i,
			"traits": ["测试能量"],
		...
		if energy_uid == "":
			return _fail("正式 life_trigger_only RAID 测试能量卡创建失败")
	var target_uid := _spawn_loaded_card(manager, UATypes.PLAYER_ONE, "UA31BT_MMM_1_078", UATypes.Zone.FRONT_LINE, true)
	var raid_uid := _spawn_loaded_card(manager, UATypes.PLAYER_ONE, "UA31BT_MMM_1_078", UATypes.Zone.HAND, true)
	if target_uid == "" or raid_uid == "":
		return _fail("正式 life_trigger_only RAID 测试卡牌创建失败")

	var actions := _get_hand_card_actions(manager, UATypes.PLAYER_ONE, raid_uid)
	if not actions.has("RAID"):
		return _fail("正式 life_trigger_only RAID 卡在主阶段满足条件时应显示 RAID 动作，实际动作: %s" % str(actions))
