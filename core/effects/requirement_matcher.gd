...
func _init_handlers() -> void:
	_handlers = {
		...
		"CONTROLLER_HAS_TRAIT_BP_GTE": _req_controller_has_trait_bp_gte,
...
func _req_controller_has_trait_bp_gte(state: GameState, requirement: Dictionary, context: Dictionary, candidate_card_uid: String, source_card_uid: String) -> bool:
	var source_card = state.get_card(source_card_uid)
	if source_card == null:
		return false
	var player = state.get_player(source_card.controller_player_id)
	if player == null:
		return false
	var trait_value := str(requirement.get("trait", ""))
	var min_bp := int(requirement.get("value", 0))
	for zone_cards in [player.front_line, player.energy_line]:
		for card_uid_v in zone_cards:
			var cuid := str(card_uid_v)
			if cuid == "":
			...
			if d == null:
				continue
			if trait_value != "" and not d.traits.has(trait_value):
				continue
			if int(c.current_bp) >= min_bp:
				return true
	return false
