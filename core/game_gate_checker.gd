...
static func current_priority_player_id(state: GameState) -> String:
	if not state.pending.decisions.is_empty():
		return str((state.pending.decisions[0] as Dictionary).get("owner_player_id", state.active_player_id))
	if not state.pending.life_triggers.is_empty():
		return str((state.pending.life_triggers[0] as Dictionary).get("player_id", state.active_player_id))
	if not state.pending.life_reveal.is_empty():
		return str(state.pending.life_reveal.get("player_id", state.active_player_id))
